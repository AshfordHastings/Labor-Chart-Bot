...
public class GetCooperCommand implements Command{
	...
	public void execute(Message message, DiscordOutput discordOutput, LaborService laborService) {
		...
		for(Member member : membersToGet) {
			try {
				Cooper cooperToQuery = laborService.getDBService().findCooperByDiscordTag(member.getAsMention());
				List<LaborSlot> laborSlotsQueried = laborService.getDBService().findLaborSlotByCooper(cooperToQuery);
